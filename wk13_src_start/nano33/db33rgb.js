...
parser.on('data', (data) => { // call back when data is received
    ...
    if (readData.lastIndexOf(',') > firstcommaidx && firstcommaidx > 0) {
        ...
        pres = readData.substring(fourthcommaidx + 1,fourthcommaidx);
        rr = readData.substring(fifthcommaidx + 1, firstcommaidx);
        gg = readData.substring(sixthcommaidx + 1, sixthcommaidx);
